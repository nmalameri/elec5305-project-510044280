...
This repository contains the full codebase, pretrained-model evaluation scripts, analysis tools, and reproducible results for my ELEC5305 deepfake speech detection project.
...
The current framework is intentionally modular and can be extended in several directions:

- **Integration of full official preprocessing pipelines**  
  Implement the complete frontend chains for RawNet2 and AASIST (pre-emphasis, sinc filtering, mel-spectrogram parameters, and model-specific normalisation) to enable fair, publication-grade evaluation of pretrained models.

- **Fine-tuning pretrained waveform models**  
  Fine-tune RawNet2, RawNetLite, or AASIST-LA directly on ASVspoof 2019 LA to resolve domain/feature mismatch and assess their true performance in matched conditions.

- **Whisper-based anti-spoofing models**  
  Incorporate Whisper-AAiST or Whisper-based embedding classifiers as additional pretrained baselines once compatible checkpoints and preprocessing pipelines are available.

- **Augmentation and robustness experiments**  
  Explore data augmentation strategies (noise, codecs, reverberation) and adversarial perturbations to analyse robustness under distorted or unseen spoofing conditions.

- **Multi-feature fusion**  
  Combine LFCC/MFCC features with raw-audio embeddings (RawNet2 or AASIST intermediate layers) to investigate hybrid systems.

- **Model distillation and lightweight deployment**  
  Distill heavier pretrained architectures into compact cepstral models for low-latency or embedded applications.

These extensions build on the existing framework and can be incorporated without modifying the core LFCC/MFCC–GMM/CNN pipeline.
